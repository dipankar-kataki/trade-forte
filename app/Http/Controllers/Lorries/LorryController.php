...
class LorryController extends Controller
{
    ...
    {
        try {
            $lorries = Lorry::with('lorry_invoices', 'lorry_items')->latest()->paginate(100);
            return $this->success("Lorry list.", $lorries, null, 200);
        } catch (\Exception $e) {
            return $this->error('Oops! Something Went Wrong.' . $e->getMessage(), null, null, 500);
        }
    }
        ...
        try {
            $lorryInvoice = Lorry::with([
                'lorry_invoices.invoices' => function ($query) {
                    $query->select('id', 'exporter_id',"exporter_address_id", 'consignee_id'); // Adjust the column names based on your actual database structure
                },
                'lorry_items'
            ])
            ->where('id', $request->id)
